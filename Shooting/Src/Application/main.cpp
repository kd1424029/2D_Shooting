...
bool Application::Init(int w, int h)
{
	...
	// ウィンドウ作成
	//===================================================================
	if (m_window.Create(w, h, "DIRECTION SHOT", "Window") == false) {
...
void Application::Execute()
{
	...
	DWORD lastTime = timeGetTime(); //前フレームの時間を記録用
	int count = 0;
	...
	while (1)
	{
		...
		//m_DeltaTimeを計算(現在の時間 - 前回の時間）
		// timeGetTimeはミリ秒(ms)で返ってくるので 1000 で割って秒に変換します
		m_DeltaTime = (float)(st - lastTime) / 1000.0f;

		// 処理落ち対策もし1フレームが長すぎたら最大0.1秒までに制限する
		if (m_DeltaTime > 0.1f) { m_DeltaTime = 0.1f; }

		lastTime = st; // 今回の時間を記録
		...
		std::string titleBar = "DIRECTION SHOT   FPS:" + std::to_string(m_fps);
